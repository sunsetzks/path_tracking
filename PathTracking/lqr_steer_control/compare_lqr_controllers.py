...
def run_comparison():
    ...
    # Plot comparison
    plot_comparison(ax, ay, cx, cy, cyaw, ck, s,
                   x_d, y_d, yaw_d, v_d, t_d, error_d,
                   x_c, y_c, yaw_c, v_c, t_c, error_c)
...
def plot_comparison(ax, ay, cx, cy, cyaw, ck, s,
                   x_d, y_d, yaw_d, v_d, t_d, error_d,
                   x_c, y_c, yaw_c, v_c, t_c, error_c):
    """
    ...
    if len(yaw_d) > len(s):
        plt.plot(t_d[:len(s)], [np.rad2deg(iyaw) for iyaw in yaw_d[:len(s)]], "-b", label="discrete LQR")
    else:
        plt.plot(t_d, [np.rad2deg(iyaw) for iyaw in yaw_d], "-b", label="discrete LQR")
    
    if len(yaw_c) > len(s):
        plt.plot(t_c[:len(s)], [np.rad2deg(iyaw) for iyaw in yaw_c[:len(s)]], "-g", label="continuous LQR")
    else:
    ...
    plt.bar(x_pos - width/2, mean_errors, width, label='Mean Error', color=['blue', 'green'], alpha=0.7)
    plt.bar(x_pos + width/2, max_errors, width, label='Max Error', color=['blue', 'green'], alpha=0.4)
...
def print_statistics(error_d, error_c, t_d, t_c):
    ...
    rms_improvement = (np.sqrt(np.mean(np.array(error_d)**2)) - np.sqrt(np.mean(np.array(error_c)**2))) / np.sqrt(np.mean(np.array(error_d)**2)) * 100
