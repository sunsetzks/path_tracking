...
if __name__ == "__main__":
    ...
    vehicle_direct_control = VehicleModel(
        steering_delay=0.3, acceleration_delay=0.2
    )  # Direct control with delays
    ...
    trajectory_direct_control = [
        vehicle_direct_control.get_state_array()
    ]  # Convert to array for plotting
    trajectory_direct_no_delay = [
        vehicle_direct_no_delay.get_state_array()
    ]  # Convert to array for plotting
    ...
    for step_index in range(time_steps):
        ...
        trajectory_direct_control.append(
            state.to_array()
        )  # Convert to array for plotting
        ...
        trajectory_direct_no_delay.append(
            state.to_array()
        )  # Convert to array for plotting
    ...
    print(
        f"   Max steering: {np.rad2deg(vehicle_with_delay.kinematics.max_steering_angle):.1f} deg"
    )
    print("\n4. VehicleModel - Complete system:")
    print(
        f"   Steering rate gain: {vehicle_direct_control.steering_rate_gain:.1f} rad/s per rad"
    )
    print(
        f"   Acceleration gain: {vehicle_direct_control.acceleration_gain:.1f} (m/s²) per (m/s)"
    )
    ...
    print(
        "Rate Control (No delay) - Position: ({:.2f}, {:.2f}) m, Yaw: {:.2f} deg, Velocity: {:.2f} m/s".format(
            trajectory_no_delay[-1, 0],
            trajectory_no_delay[-1, 1],
            np.rad2deg(trajectory_no_delay[-1, 2]),
            trajectory_no_delay[-1, 3],
        )
    )
    print(
        "Rate Control (With delay) - Position: ({:.2f}, {:.2f}) m, Yaw: {:.2f} deg, Velocity: {:.2f} m/s".format(
            trajectory_with_delay[-1, 0],
            trajectory_with_delay[-1, 1],
            np.rad2deg(trajectory_with_delay[-1, 2]),
            trajectory_with_delay[-1, 3],
        )
    )
    print(
        "Direct Control (With delay) - Position: ({:.2f}, {:.2f}) m, Yaw: {:.2f} deg, Velocity: {:.2f} m/s".format(
            trajectory_direct_control[-1, 0],
            trajectory_direct_control[-1, 1],
            np.rad2deg(trajectory_direct_control[-1, 2]),
            trajectory_direct_control[-1, 3],
        )
    )
