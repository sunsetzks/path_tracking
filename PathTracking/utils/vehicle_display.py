...
class VehicleDisplay:
    ...
    def _rotate_and_translate(self, points: np.ndarray, 
...
def demo_vehicle_display():
    ...
    fig.suptitle('Vehicle Display Demo - Four Wheels with Steering Visualization', 
    ...
    ax1 = axes[0, 0]
    display.plot_vehicle(0, 0, 0, 0, ax=ax1, body_color='blue', 
                        front_wheel_color='red', wheel_color='black')
    ...
    # Demo 2: Left turn
    ax2 = axes[0, 1]
    display.plot_vehicle(0, 0, 0, math.pi/6, ax=ax2, body_color='green',
                        front_wheel_color='orange', wheel_color='darkgray')
    ...
    # Demo 3: Right turn
    ax3 = axes[1, 0]
    display.plot_vehicle(0, 0, 0, -math.pi/4, ax=ax3, body_color='purple',
                        front_wheel_color='yellow', wheel_color='brown')
    ...
    ax4 = axes[1, 1]
    display.plot_vehicle(0, 0, math.pi/3, math.pi/8, ax=ax4, body_color='red',
                        front_wheel_color='cyan', wheel_color='magenta')
...
def demo_vehicle_trajectory(ax: Optional[plt.Axes] = None):
    ...
    for i in range(0, len(t), 4):  # Every 4th point
        ...
        display.plot_vehicle(x, y, yaw, steering_angle, ax=ax,
                           body_color=color, alpha=0.6,
                           show_direction_arrow=True)
    
    ax.set_title('Vehicle Following Circular Trajectory\n(Shows steering angles for turning)', 
