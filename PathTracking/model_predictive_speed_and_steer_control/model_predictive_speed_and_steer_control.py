...
def plot_vehicle(x, y, yaw, steering_angle=0.0, body_color="-r", wheel_color="-k"):
    ...
    vehicle_outline = np.array([
        [-REAR_TO_WHEEL_DISTANCE, (VEHICLE_LENGTH - REAR_TO_WHEEL_DISTANCE), 
         (VEHICLE_LENGTH - REAR_TO_WHEEL_DISTANCE), -REAR_TO_WHEEL_DISTANCE, -REAR_TO_WHEEL_DISTANCE],
    ...
    front_right_wheel = np.array([
        ...
        [-WHEEL_WIDTH - WHEEL_TREAD, -WHEEL_WIDTH - WHEEL_TREAD, 
         WHEEL_WIDTH - WHEEL_TREAD, WHEEL_WIDTH - WHEEL_TREAD, -WHEEL_WIDTH - WHEEL_TREAD]
    ])
    ...
    for component in [vehicle_outline, front_right_wheel, front_left_wheel, rear_right_wheel, rear_left_wheel]:
        component[0, :] += x
        component[1, :] += y

    # Plot all components
    plt.plot(np.array(vehicle_outline[0, :]).flatten(),
             np.array(vehicle_outline[1, :]).flatten(), body_color)
    plt.plot(np.array(front_right_wheel[0, :]).flatten(),
             np.array(front_right_wheel[1, :]).flatten(), wheel_color)
    plt.plot(np.array(rear_right_wheel[0, :]).flatten(),
             np.array(rear_right_wheel[1, :]).flatten(), wheel_color)
    plt.plot(np.array(front_left_wheel[0, :]).flatten(),
             np.array(front_left_wheel[1, :]).flatten(), wheel_color)
    plt.plot(np.array(rear_left_wheel[0, :]).flatten(),
             np.array(rear_left_wheel[1, :]).flatten(), wheel_color)
    plt.plot(x, y, "*")
...
def predict_vehicle_motion(initial_state, acceleration_sequence, steering_sequence, reference_trajectory):
    ...
    # Simulate forward
    current_state = VehicleState(x=initial_state[0], y=initial_state[1], 
                                yaw=initial_state[3], velocity=initial_state[2])
...
def iterative_linear_mpc_control(reference_trajectory, initial_state, reference_steering, 
                               previous_acceleration, previous_steering):
    """
    ...
    for iteration in range(MAX_ITERATIONS):
        ...
        predicted_trajectory = predict_vehicle_motion(initial_state, previous_acceleration, 
                                                    previous_steering, reference_trajectory)
        ...
        (previous_acceleration, previous_steering, predicted_x, predicted_y, 
         predicted_yaw, predicted_velocity) = solve_linear_mpc(reference_trajectory, predicted_trajectory, 
                                                              initial_state, reference_steering)
        
        # Check convergence
        control_change = (sum(abs(np.array(previous_acceleration) - np.array(old_acceleration))) + 
    ...
    return (previous_acceleration, previous_steering, predicted_x, predicted_y, 
            predicted_yaw, predicted_velocity)
...
def solve_linear_mpc(reference_trajectory, linearization_trajectory, initial_state, reference_steering):
    ...
    for time_step in range(PREDICTION_HORIZON):
        ...
        if time_step < (PREDICTION_HORIZON - 1):
            ...
            constraints += [cvxpy.abs(control_variables[1, time_step + 1] - 
                                    control_variables[1, time_step]) <= steering_rate_limit]
    ...
    return acceleration_sequence, steering_sequence, predicted_x, predicted_y, predicted_yaw, predicted_velocity


def calculate_reference_trajectory(current_state, trajectory_x, trajectory_y, trajectory_yaw, 
                                 trajectory_curvature, speed_profile, path_resolution, previous_target_index):
    """
    ...
    nearest_index, _ = find_nearest_trajectory_index(current_state, trajectory_x, trajectory_y, 
                                                    trajectory_yaw, previous_target_index)
...
def check_goal_reached(current_state, goal_position, target_index, trajectory_length):
    ...
    distance_to_goal = math.hypot(current_state.x - goal_position[0], 
                                 current_state.y - goal_position[1])
...
def run_mpc_simulation(trajectory_x, trajectory_y, trajectory_yaw, trajectory_curvature, 
                      speed_profile, path_resolution, initial_state):
    """
    ...
    while simulation_time <= MAX_SIMULATION_TIME:
        ...
        reference_trajectory, target_index, reference_steering = calculate_reference_trajectory(
            current_state, trajectory_x, trajectory_y, trajectory_yaw, trajectory_curvature,
            speed_profile, path_resolution, target_index)

        # Current state vector
        current_state_vector = [current_state.x, current_state.y, current_state.velocity, current_state.yaw]

        # Solve MPC optimization
        (acceleration_sequence, steering_sequence, predicted_x, predicted_y, 
         predicted_yaw, predicted_velocity) = iterative_linear_mpc_control(
            reference_trajectory, current_state_vector, reference_steering, 
            previous_acceleration_sequence, previous_steering_sequence)
        ...
        if SHOW_ANIMATION:
            ...
            plt.plot(reference_trajectory[0, :], reference_trajectory[1, :], "xk", label="Reference")
            plt.plot(trajectory_x[target_index], trajectory_y[target_index], "xg", label="Target")
            
            plot_vehicle(current_state.x, current_state.y, current_state.yaw, steering_angle=applied_steering)
    ...
    return (time_history, x_history, y_history, yaw_history, velocity_history, 
            steering_history, acceleration_history)
...
def create_straight_path(path_resolution):
    ...
    trajectory_x, trajectory_y, trajectory_yaw, trajectory_curvature, _ = \
        cubic_spline_planner.calc_spline_course(waypoints_x, waypoints_y, ds=path_resolution)
...
def create_parking_path(path_resolution):
    ...
    forward_x, forward_y, forward_yaw, forward_curvature, _ = \
        cubic_spline_planner.calc_spline_course(forward_waypoints_x, forward_waypoints_y, ds=path_resolution)
    ...
    reverse_x, reverse_y, reverse_yaw, reverse_curvature, _ = \
        cubic_spline_planner.calc_spline_course(reverse_waypoints_x, reverse_waypoints_y, ds=path_resolution)
...
def main():
    ...
    initial_state = VehicleState(x=trajectory_x[0], y=trajectory_y[0], 
                                yaw=trajectory_yaw[0], velocity=0.0)

    # Run simulation
    (time_history, x_history, y_history, yaw_history, velocity_history, 
     steering_history, acceleration_history) = run_mpc_simulation(
    ...
    if SHOW_ANIMATION:
        ...
        # Plot speed profile
        plt.subplot(2, 2, 2)
        plt.plot(time_history, [v * 3.6 for v in velocity_history], "-r", label="Vehicle speed", linewidth=2)
        plt.grid(True)
        plt.xlabel("Time [s]")
        ...
        # Plot steering angle
        plt.subplot(2, 2, 3)
        plt.plot(time_history[1:], [math.degrees(s) for s in steering_history[1:]], 
                "-b", label="Steering angle", linewidth=2)
        plt.grid(True)
        plt.xlabel("Time [s]")
        ...
        # Plot acceleration
        plt.subplot(2, 2, 4)
        plt.plot(time_history[1:], acceleration_history[1:], "-m", label="Acceleration", linewidth=2)
        plt.grid(True)
        plt.xlabel("Time [s]")
