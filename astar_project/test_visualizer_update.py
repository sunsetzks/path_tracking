...
import asyncio
import numpy as np
from astar_project.hybrid_astar import State, DirectionMode, VehicleModel, HybridAStar
...
if __name__ == "__main__":
    print("🚗 Hybrid A* Visualizer Update Test")
    print("=" * 40)
    
    # Check if running in async environment
    try:
        asyncio.run(test_updated_visualizer())
    except KeyboardInterrupt:
        print("\nTest interrupted by user")
    except Exception as e:
        print(f"Error during test: {e}")
        print("Falling back to matplotlib test...")
        test_matplotlib_fallback()
