 ...
 * @brief Simple standalone demo without dependencies
 */
...
int main() {
    std::cout << "=== Hybrid A* Simple Planning Demo ===" << std::endl;
    ...
    if (path_nodes.has_value()) {
        ...
        // Print last few path points
        std::cout << "\nLast 5 path points:" << std::endl;
        size_t start_idx = detailed_path.size() > 5 ? detailed_path.size() - 5 : 0;
        for (size_t i = start_idx; i < detailed_path.size(); ++i) {
            const auto& point = detailed_path[i];
            std::cout << "  " << i << ": (" << point.x << ", " << point.y << ", " << point.yaw << ")" << std::endl;
        }
